  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6 mt-12">
      {data.map((product) => (
        ...
        >
          <Link href={`/products/${product.id}`} className="block"> {/* Link to dynamic route */}
            <img
              src={product.image}
              alt={product.title}
            ...
            <div className="product-details p-4">
              <h2 className="product-title text-lg font-semibold mb-2">{product.title}</h2>
              <p className="product-category text-sm text-gray-500 mb-2">
                Category: {product.category}
              </p>
              <p className="product-description text-sm text-gray-700 mb-4">
                {product.description}
              </p>
